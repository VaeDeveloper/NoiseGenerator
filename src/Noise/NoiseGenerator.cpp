...
float* StupidNoise1D(int res, int freq, float* data2, float scale)
{
	...
	if(!data1 || !data2) throw std::runtime_error("Out of memory");
...
float* PerlinNoise1D(int res, const noise_properties* noise_properties)
{
	float* data = NULL;
	float  scale = 1.0f;
	int    freq = 2;
...
float* SphereNoise(const float* input_noise, int input_res, float* output_noise, int output_res, int face_index)
{
	if(!output_noise)
	{
		...
		if(!output_noise) throw std::runtime_error("Out of memory");
	}
